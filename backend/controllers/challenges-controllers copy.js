...
const getChallengeById = async (req, res, next) => {
  ...
  connection.query(sql, [challengeID], (err, result) => {
    if (result.length === 0) {
...
const updateChallenge = async (req, res, next) => {
  ...
  const sql = `UPDATE challenges SET name=?, description=?, difficulty=?, imageURL=? WHERE challengeID=?`;
  const values = [name, description, difficulty, imageURL, challengeID];
  connection.query(sql, values, (err, result) => {
    if (err) {
      if (err.code === "ER_DUP_ENTRY") {
        return next(new Error("Challenge name already exists. Please choose a different name."));
      }
    ...
    return next(new Error("Challenge updated successfully!"));
  });
};
